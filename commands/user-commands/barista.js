    ...
    try {
      ...
      if (subcommand === 'training') {
        ...
        if (trueNpc) {
          ...
          logger.log(`[DEBUG] customer.fateOrder:`, customer.fateOrder);
          theFatefulOrder = JsonSearchServices.findBaristaKapéDrinkItem(customer.fateOrder.drink.id);
          theFatefulOrder = theFatefulOrder.found;
          logger.log(`[DEBUG] theFatefulOrder:`, theFatefulOrder);
          ...
          //logger.log(`[DEBUG] barista.current_customer_relationship:`, barista.current_customer_relationship);
        }
            ...
            if (!isSelf) {
              ...
              try {
                ...
                if (precise) {
                  ...
                  logger.log(`[DEBUG] letTheRecordState:`, letTheRecordState);
                  ...
                  logger.log(`[LOG] Barista Relationship Level Up:`, barista.current_customer_relationship.status, barista.current_customer_relationship.level);
                  await wait(1_000);
                  const bonus = {
                    exp: 750,
                    cash: 25
                  ...
                  await interaction.followUp({
                    content: `Your relationship with ${customer[`${barista.prev_relationship.status}`].name} has grown. You are now **${FormatServices.capitalizeFirstLetter(barista.current_customer_relationship.status)} Level ${barista.current_customer_relationship.level}**\n\n-# **+${bonus.exp} experience**\n-# **+${displayBonusCash.amount} ${displayBonusCash.units}**`
                  });
                  if (!baristaLevelUp) return collector.stop('End of order.');
                } if (baristaLevelUp) {
